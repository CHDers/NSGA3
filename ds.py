...
for _ in range(gen_size):
    ...
    for vec in ref_vectors:
        ...
        # 根据权重向量vec，计算出一个参考点ref_point，在目标函数空间中代表着该权重下的理想解。
        weighted_objs = [(ind.objs[k] - ref_point[k]) * vec[k]
                         for ind in pop_for_cross for k in range(num_obj)]
        # 对于当前的所有个体，在目标函数空间中的加权距离进行排序。
        sorted_objs = sorted(weighted_objs)
        # 在排序后的加权距离列表中选择中位数值，并将其作为拥挤度距离的计算基准。
        median_objs = [sorted_objs[len(sorted_objs) // 2 + offset]
                       for offset in (-1, 0, 1)]
...
# 可视化
plt.scatter([ind.objs[0] for ind in pop], [ind.objs[1]
                                           for ind in pop], c='gray', alpha=0.5)
plt.scatter([ind.objs[0] for ind in pareto_front], [ind.objs[1]
                                                    for ind in pareto_front], c='r')
